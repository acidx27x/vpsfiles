...
install_packages() {
  ...
  if [[ "${os_id}" == "ubuntu" || "${os_like}" == *"ubuntu"* ]]; then
    ...
    # Official AmneziaWG Debian instructions currently use the Ubuntu focal PPA.
  ...
  command -v awg >/dev/null 2>&1 || die "awg is missing after installing amneziawg"
  command -v awg-quick >/dev/null 2>&1 || die "awg-quick is missing after installing amneziawg"
}
...
setup_firewall() {
  ...
  if ! ufw status | grep -q "Status: active"; then
    if confirm "UFW is not active. Enable it now?"; then
      ...
      echo "Skipped enabling UFW. The AmneziaWG UDP port was still added to UFW rules."
    fi
  fi
}

start_amneziawg() {
  systemctl enable "awg-quick@${AWG_IF}" >/dev/null
...
print_summary() {
  ...
  echo "  sudo ./add-peer.sh phone               # add an existing client to ${AWG_IF}.conf"
  ...
  echo "  sudo ./remove-peer.sh phone            # remove an existing client from ${AWG_IF}.conf"
