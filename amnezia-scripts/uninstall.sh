#!/usr/bin/env bash
set -euo pipefail

# Remove AmneziaWG state created by this script bundle.
# This does not uninstall OS packages or remove WireGuard configs.

AWG_IF_DEFAULT="awg0"
AWG_DIR="/etc/amnezia/amneziawg"
SYSCTL_FILE="/etc/sysctl.d/99-amneziawg.conf"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CLIENTS_DIR="${SCRIPT_DIR}/clients"
BACKUP_ROOT="${BACKUP_ROOT:-${SCRIPT_DIR}/install-backups}"

AWG_IF="${AWG_IF:-${AWG_IF_DEFAULT}}"

require_root() {
  if [[ "${EUID}" -ne 0 ]]; then
    echo "ERROR: run as root:"
    echo "  sudo bash ${0}"
    exit 1
  fi
}

confirm() {
  local message="$1"
  local answer=""

  read -r -p "${message} [y/N]: " answer
  case "${answer}" in
    y|Y|yes|YES) return 0 ;;
    *) return 1 ;;
  esac
}

remove_path() {
  local path="$1"

  [[ -n "${path}" && "${path}" != "/" ]] || {
    echo "ERROR: refusing to remove unsafe path: ${path}" >&2
    exit 1
  }

  if [[ -e "${path}" ]]; then
    rm -rf "${path}"
    echo "Removed: ${path}"
  fi
}

remove_client_files() {
  [[ -d "${CLIENTS_DIR}" ]] || return 0
  [[ "${CLIENTS_DIR}" == "${SCRIPT_DIR}/clients" ]] || {
    echo "ERROR: refusing to clean unexpected clients path: ${CLIENTS_DIR}" >&2
    exit 1
  }

  find "${CLIENTS_DIR}" -mindepth 1 ! -name .gitkeep -exec rm -rf {} +
  echo "Removed generated client files from: ${CLIENTS_DIR}"
}

stop_amneziawg() {
  if command -v systemctl >/dev/null 2>&1; then
    systemctl stop "awg-quick@${AWG_IF}" 2>/dev/null || true
    systemctl disable "awg-quick@${AWG_IF}" 2>/dev/null || true
  fi

  if command -v awg-quick >/dev/null 2>&1; then
    awg-quick down "${AWG_IF}" 2>/dev/null || true
  fi
}

remove_ufw_rule() {
  local port_file="${SCRIPT_DIR}/server-port.txt"
  local port=""

  [[ -f "${port_file}" ]] || return 0
  port="$(cat "${port_file}")"
  [[ -n "${port}" ]] || return 0

  if command -v ufw >/dev/null 2>&1; then
    ufw delete allow "${port}/udp" >/dev/null 2>&1 || true
  fi
}

print_plan() {
  echo "This will remove AmneziaWG data created by this script bundle:"
  echo
  echo "  Interface service: awg-quick@${AWG_IF}"
  echo "  Server config:     ${AWG_DIR}/${AWG_IF}.conf"
  echo "  Server keys:       ${AWG_DIR}/server_private_key, ${AWG_DIR}/server_public_key"
  echo "  Sysctl file:       ${SYSCTL_FILE}"
  echo "  Client files:      ${CLIENTS_DIR} contents except .gitkeep"
  echo "  Install backups:   ${BACKUP_ROOT}"
  echo "  Script state:      last-ip.txt, last-ip6.txt, server-endpoint.txt,"
  echo "                     server-endpoint6.txt, server-port.txt, server-interface.txt,"
  echo "                     server-net.txt, server-net6.txt, obfuscation.env"
  echo
  echo "It will also try to remove the UFW allow rule for the saved AmneziaWG UDP port."
  echo "It will not uninstall apt packages or remove normal WireGuard files."
}

main() {
  require_root

  print_plan
  echo
  if ! confirm "Continue with uninstall?"; then
    echo "Aborted before making changes."
    exit 1
  fi

  stop_amneziawg
  remove_ufw_rule

  remove_path "${AWG_DIR}/${AWG_IF}.conf"
  remove_path "${AWG_DIR}/server_private_key"
  remove_path "${AWG_DIR}/server_public_key"
  remove_path "${SYSCTL_FILE}"

  remove_client_files
  remove_path "${BACKUP_ROOT}"
  remove_path "${SCRIPT_DIR}/last-ip.txt"
  remove_path "${SCRIPT_DIR}/last-ip6.txt"
  remove_path "${SCRIPT_DIR}/server-endpoint.txt"
  remove_path "${SCRIPT_DIR}/server-endpoint6.txt"
  remove_path "${SCRIPT_DIR}/server-port.txt"
  remove_path "${SCRIPT_DIR}/server-interface.txt"
  remove_path "${SCRIPT_DIR}/server-net.txt"
  remove_path "${SCRIPT_DIR}/server-net6.txt"
  remove_path "${SCRIPT_DIR}/obfuscation.env"

  if command -v sysctl >/dev/null 2>&1; then
    sysctl --system >/dev/null 2>&1 || true
  fi

  echo "Uninstall complete."
}

main "$@"
